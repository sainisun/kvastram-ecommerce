...
import { Package, DollarSign, Clock, CheckCircle, AlertCircle, ChevronLeft, ChevronRight, Search, Filter } from 'lucide-react';
...
export default function WholesaleOrdersPage() {
    ...
    const fetchOrders = async () => {
        try {
            ...
            const [ordersData, statsData] = await Promise.all([
                api.getWholesaleOrders(statusFilter === 'all' ? undefined : statusFilter, page),
                api.getWholesaleOrderStats()
            ]);
            setOrders(ordersData.orders || []);
            setPagination(ordersData.pagination || { page: 1, limit: 20, total: 0, pages: 1 });
            setStats(statsData || { total: 0, pending: 0, processing: 0, completed: 0, total_value: 0 });
        } catch (error) {
    ...
    const getPaymentStatusBadge = (status: string) => {
        ...
        return (
            <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${styles[status] || 'bg-gray-100 text-gray-800'}`}>
                {status === 'awaiting' ? 'Awaiting Payment' : (status || 'unknown').charAt(0).toUpperCase() + (status || 'unknown').slice(1)}
            </span>
        );
    };
