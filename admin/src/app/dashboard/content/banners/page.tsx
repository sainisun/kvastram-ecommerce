...
import { Plus, Trash2, ExternalLink, Image as ImageIcon, Edit2, ChevronUp, ChevronDown, X, GripVertical } from 'lucide-react';
    ...
    return (
        <div className="p-6">
                        ...
                        <form onSubmit={handleCreate} className="space-y-4">
                            ...
                            <div>
                                ...
                                <div className="flex items-center gap-4">
                                    {newItem.image_url && (
                                        <img src={newItem.image_url} alt="Preview" className="h-32 w-48 object-cover rounded-lg border" />
                                    )}
                ...
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                    <div className="bg-white p-6 rounded-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
                        <div className="flex justify-between items-center mb-4">
                            ...
                            <button onClick={() => { setEditing(false); setEditingBanner(null); }} className="text-gray-400 hover:text-gray-600">
                                <X size={24} />
                            </button>
                        </div>
                        <form onSubmit={handleUpdate} className="space-y-4">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div>
                                    ...
                                    <select
                                        ...
                                        onChange={e => setEditingBanner({ ...editingBanner, section: e.target.value })}
                                        className="w-full px-3 py-2 border rounded-lg"
                                    >
                                        <option value="hero">Hero Slider</option>
                                        <option value="promo">Promo Banner</option>
                            ...
                            <div>
                                ...
                                <select
                                    ...
                                    onChange={e => setEditingBanner({ ...editingBanner, is_active: e.target.value === 'true' })}
                                    className="w-full px-3 py-2 border rounded-lg"
                                >
                            ...
                            <div>
                                ...
                                <div className="flex items-center gap-4">
                                    {editingBanner.image_url && (
                                        <img src={editingBanner.image_url} alt="Preview" className="h-32 w-48 object-cover rounded-lg border" />
                                    )}
                                    <label className="cursor-pointer bg-gray-100 px-4 py-2 rounded-lg hover:bg-gray-200 transition">
                                        <span className="flex items-center gap-2">
                                            <ImageIcon size={18} />
                            ...
                            <div className="flex justify-end gap-2 pt-4 border-t">
                                <button
                                    type="button"
                                    onClick={() => { setEditing(false); setEditingBanner(null); }}
                                    className="px-4 py-2 text-gray-600 hover:bg-gray-100 rounded-lg"
                                >
                                    Cancel
                                </button>
            ...
            {loading ? (
                ...
                <div className="text-center py-12 text-gray-500">No banners found. Create one to get started.</div>
            ) : (
                <div className="space-y-4">
                    {banners.map((banner, index) => (
                        <div key={banner.id} className="flex items-center bg-white p-4 rounded-xl border border-gray-200 shadow-sm gap-4">
                            {reordering && (
                            ...
                            <img src={banner.image_url} alt={banner.title} className="h-16 w-32 object-cover rounded-lg bg-gray-100" />
                            <div className="flex-1">
                                <h3 className="font-bold text-gray-800">{banner.title}</h3>
                                <p className="text-sm text-gray-500">
                                    {banner.section} • {banner.is_active ? <span className="text-green-600">Active</span> : <span className="text-red-500">Inactive</span>}
                                </p>
                                {banner.link && <div className="text-xs text-blue-600 flex items-center gap-1 mt-1"><ExternalLink size={10} /> {banner.link}</div>}
                            </div>
