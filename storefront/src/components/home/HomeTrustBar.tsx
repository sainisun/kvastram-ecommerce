...
export function HomeTrustBar() {
  return (
    <section className="bg-[var(--ds-surface-paper)] border-b border-border-subtle" aria-label="Odhvica shopping promises">
          ...
          {trustItems.map((item) => {
            ...
            return (
              <div key={item.label} className="flex-none flex items-center gap-[8px] p-0 border-0 min-h-auto md:grid md:grid-cols-[auto_1fr] md:gap-[12px] md:min-h-[92px] md:p-[18px] md:border-r md:border-border-subtle md:last:border-r-0">
                ...
                <div>
                  <strong className="block font-label text-[11px] md:text-[var(--ds-text-body-sm)] font-[var(--ds-type-label-weight)] tracking-[var(--ds-type-label-tracking)] leading-[var(--ds-leading-snug)] uppercase text-primary whitespace-nowrap md:whitespace-normal">{item.label}</strong>
                  <p className="hidden md:block md:mt-[4px] md:font-body md:text-[var(--ds-text-body-xs)] md:leading-[var(--ds-leading-normal)] md:text-muted">{item.copy}</p>
                </div>
              </div>
            );
          })}
