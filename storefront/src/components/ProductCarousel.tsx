...
export default function ProductCarousel({ 
    ...
    const handlePrev = () => {
        const newIndex = currentIndex > 0 ? currentIndex - 1 : Math.max(0, products.length - 4);
    ...
    if (products.length === 0) {
        return <div className="py-20 text-center text-stone-500 font-light italic">No products found.</div>;
    }
                ...
                {products.map((product) => (
                    ...
                    >
                        ...
                        <Link href={`/products/${product.handle || product.id}`} className="space-y-2 text-center mt-auto">
                            ...
                            <h3 className="font-serif text-lg text-stone-900 leading-tight group-hover:text-stone-600 transition-colors line-clamp-2">{product.title}</h3>
