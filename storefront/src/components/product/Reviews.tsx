...
export function Reviews({ productId }: ReviewsProps) {
    const [reviews, setReviews] = useState<Array<{ id: string; title?: string; author_name: string; content: string; rating: number; created_at: string; images?: string[] }>>([]);
    ...
    const uploadImages = async (): Promise<string[]> => {
        ...
        try {
            ...
            const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/reviews/upload`, {
                method: 'POST',
            ...
            <div className="max-w-7xl mx-auto px-6">
                ...
                {showForm && !submitted && (
                    <form onSubmit={handleSubmit} className="max-w-xl bg-stone-50 p-8 mb-12 rounded-sm space-y-4">
                        ...
                        >
                            {(submitting || uploadingImages) && <Loader2 className="animate-spin" size={16} />}
                                ...
                                {review.images && review.images.length > 0 && (
                                    <div className="flex flex-wrap gap-2 mb-4">
                                        {review.images.map((imageUrl, imgIndex) => (
                                            <div key={imgIndex} className="w-20 h-20 relative rounded overflow-hidden border border-stone-200">
                                                <Image
                                                    src={imageUrl}
