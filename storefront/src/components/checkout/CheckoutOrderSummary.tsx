...
import { Button, Input, OptimizedImage, PaymentIcons, SecurityBadges } from '@/design-system';
...
export interface CheckoutOrderSummaryProps {
  ...
  finalTotal: number;
  hasConfirmedOrderTotals: boolean;
...
export default function CheckoutOrderSummary({
  ...
  giftWrappingCost,
  finalTotal,
  hasConfirmedOrderTotals,
  displayMoney,
  displayConfirmedMoney,
}: CheckoutOrderSummaryProps) {
  const formatAmount = (amount: number) =>
    hasConfirmedOrderTotals ? displayConfirmedMoney(amount) : displayMoney(amount);
      ...
      <div className="max-w-lg mx-auto sticky top-24">
        ...
        {step === 'shipping' && (
          <div className="mb-6 border-b border-border-subtle pb-6">
            ...
            {promoMessage && (
              <p className={`text-body-xs mt-2 ${promoMessage.type === 'success' ? 'text-success' : 'text-error'}`}>
                {promoMessage.text}
              </p>
            )}
          </div>
        ...
        <div className="border-t border-border-subtle pt-6 space-y-3 text-body-sm">
          <div className="flex justify-between text-muted">
            <span>Subtotal</span>
            <span>{displayMoney(cartTotal)}</span>
          </div>
          {discount && (
            <div className="flex justify-between text-success">
              ...
              <span>-{displayMoney(discount.amount)}</span>
            </div>
          )}
            ...
            <div className="flex justify-between text-muted">
              ...
              <span>
                {taxLoading ? (
                  ...
                  formatAmount(displayedTaxAmount)
                )}
              </span>
            </div>
          )}
          {giftWrapping && (
            <div className="flex justify-between text-muted">
              ...
              <span>{formatAmount(giftWrappingCost)}</span>
            </div>
          )}
        ...
        <div className="mt-8 flex gap-3 border border-border-subtle bg-surface-paper p-4 text-body-xs text-muted">
          ...
          <p>
            Every purchase is backed by our Authenticity Guarantee. We ensure the highest standards of craftsmanship.
          </p>
        </div>
