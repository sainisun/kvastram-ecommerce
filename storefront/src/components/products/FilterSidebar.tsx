...
export default function FilterSidebar({
  ...
  return (
    <div
      className={`w-64 flex-shrink-0 border-r border-stone-100 pr-8 ${className}`}
    >
                  ...
                  <div className="pl-4 border-l border-stone-200 mt-1 mb-2 space-y-1">
                    {cat.children.map((sub) => (
                      <button
                        ...
                        className={`block w-full text-left py-1 text-xs hover:text-black transition-colors ${
          ...
          <div className="space-y-1">
            {collections.map((col) => (
              <button
                ...
                className={`block w-full text-left py-1 text-sm hover:text-black transition-colors ${
        ...
        <div className="flex flex-wrap gap-2">
          {tags.map((tag) => (
            <button
              key={tag.id}
              onClick={() =>
                updateFilter('tag_id', currentTagId === tag.id ? null : tag.id)
              }
