...
export function StarRating({ rating, onRatingChange, editable = false, size = 16 }: StarRatingProps) {
    ...
    return (
        <div
            ...
            aria-label={editable ? `Rating: ${rating} out of 5 stars` : `${rating} stars`}
        >
            {[1, 2, 3, 4, 5].map((star) => (
                <button
                    key={star}
                ...
                >
                    <Star
                        ...
                        className={(hoverRating || rating) >= star ? 'text-yellow-500' : 'text-stone-300'}
                    />
                </button>
            ))}
        </div>
