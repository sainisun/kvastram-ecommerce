...
export default function CountrySelect({ 
    ...
    const common = filteredCountries.filter(c => commonCountries.includes(c.code));
    const others = filteredCountries.filter(c => !commonCountries.includes(c.code));
                                ...
                                {common.map((country) => (
                                    <button
                                        ...
                                        className={`w-full px-4 py-2.5 text-left text-sm flex items-center justify-between hover:bg-stone-50 ${
                                            value === country.code ? 'bg-stone-100 text-stone-900 font-medium' : 'text-stone-700'
                                        }`}
                                    >
                                        <span>{country.name}</span>
                                        {value === country.code && <Check size={16} className="text-stone-900" />}
                                    </button>
                                ))}
                            </>
                        )}
