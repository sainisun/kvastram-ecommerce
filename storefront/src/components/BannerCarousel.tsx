...
export default function BannerCarousel({ banners }: { banners: Banner[] }) {
    ...
    const heroBanners = banners.filter(b => b.section === 'hero').sort((a, b) => a.position - b.position);
    ...
    const nextSlide = () => setCurrentIndex((prev) => (prev + 1) % heroBanners.length);
    const prevSlide = () => setCurrentIndex((prev) => (prev - 1 + heroBanners.length) % heroBanners.length);
