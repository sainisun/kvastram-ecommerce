...
export async function Footer() {
  ...
  const supportLinks = pages.filter((p: any) => 
    ['contact', 'shipping-returns', 'faq', 'privacy-policy', 'terms-of-service'].includes(p.slug)
  );
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-12">
          ...
          <div>
            ...
            <ul className="space-y-2 text-sm text-stone-400">
              ...
              <li><Link href="/products?sort=created_at" className="hover:text-white transition-colors">Best Sellers</Link></li>
              <li><Link href="/collections" className="hover:text-white transition-colors">Collections</Link></li>
              <li><Link href="/products?tag=accessories" className="hover:text-white transition-colors">Accessories</Link></li>
            </ul>
          </div>
