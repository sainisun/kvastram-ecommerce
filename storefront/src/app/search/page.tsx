...
function SearchContent() {
    ...
    const [appliedFilters, setAppliedFilters] = useState<{ min?: string; max?: string }>({});
    ...
    useEffect(() => {
        const fetchResults = async () => {
            ...
            try {
                ...
                const params: { search: string; limit: number; sort: string; min_price?: number; max_price?: number } = {
                    search: query,
                    limit: 50,
                    sort: sort
                };
                if (appliedFilters.min) params.min_price = Number(appliedFilters.min) * 100;
                if (appliedFilters.max) params.max_price = Number(appliedFilters.max) * 100;
    ...
    return (
        <div className="min-h-screen bg-white pt-24 pb-20">
            <div className="max-w-7xl mx-auto px-6 lg:px-8">
                {/* Header */}
                <div className="mb-12">
                    <Link href="/" className="inline-flex items-center gap-2 text-xs text-stone-500 hover:text-black mb-6 uppercase tracking-widest pl-1">
                    ...
                    <p className="text-stone-500">
                        {loading ? (
                            <span className="flex items-center gap-2"><Loader2 className="animate-spin" size={16} /> Searching for &quot;{query}&quot;...</span>
                        ) : (
                            <span>Found {products.length} results for &quot;<span className="text-black font-medium">{query}</span>&quot;</span>
                        )}
                    </p>
                </div>
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between border-y border-stone-100 py-4 mb-12 gap-4">
                    ...
                    <div className="relative">
                        <button
                            ...
                            className="flex items-center gap-2 text-xs font-bold uppercase tracking-widest hover:text-stone-600"
                        >
                            Sort by: {sort === 'relevance' ? 'Featured' : sort === 'price_asc' ? 'Price: Low to High' : sort === 'price_desc' ? 'Price: High to Low' : 'Newest'}
                        </button>
                        ...
                        {showSortMenu && (
                            <div className="absolute right-0 top-full mt-2 w-48 bg-white shadow-xl z-20 border border-stone-100 rounded-sm overflow-hidden py-1">
                                <button onClick={() => { setSort('relevance'); setShowSortMenu(false); }} className="w-full text-left px-4 py-2 hover:bg-stone-50 text-sm">Featured</button>
                                <button onClick={() => { setSort('newest'); setShowSortMenu(false); }} className="w-full text-left px-4 py-2 hover:bg-stone-50 text-sm">Newest Arrivals</button>
                                <button onClick={() => { setSort('price_asc'); setShowSortMenu(false); }} className="w-full text-left px-4 py-2 hover:bg-stone-50 text-sm">Price: Low to High</button>
                                <button onClick={() => { setSort('price_desc'); setShowSortMenu(false); }} className="w-full text-left px-4 py-2 hover:bg-stone-50 text-sm">Price: High to Low</button>
                            </div>
                        )}
                    </div>
                </div>
                ...
                {loading ? (
                    ...
                    <div className="text-center py-20 bg-stone-50 rounded-lg">
                        <p className="text-stone-500 mb-4">No products found matching your search.</p>
                        <Link href="/products" className="inline-block bg-stone-900 text-white px-8 py-3 text-xs font-bold uppercase tracking-widest hover:bg-stone-800">
                            Browse All Products
                        </Link>
                    </div>
                )}
