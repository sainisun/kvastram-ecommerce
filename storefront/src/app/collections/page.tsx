    ...
    return (
      <div className="min-h-screen bg-white">
        ...
        <div className="max-w-7xl mx-auto px-6 py-24 text-center">
          <p className="text-stone-500 text-lg">No collections found. Check back soon!</p>
        </div>
      </div>
    );
  }
          ...
          <div key={col.id} className={`flex flex-col md:flex-row gap-12 items-center ${idx % 2 === 1 ? 'md:flex-row-reverse' : ''}`}>
            <div className="w-full md:w-1/2 aspect-[4/3] bg-stone-100 relative group overflow-hidden cursor-pointer">
              {col.image ? (
                ...
                <div className="absolute inset-0 bg-stone-200 group-hover:scale-105 transition-transform duration-700 flex items-center justify-center">
                  <span className="text-stone-400 font-serif italic text-xl">{col.title}</span>
                </div>
              )}
            </div>
            <div className="w-full md:w-1/2 space-y-6 text-center md:text-left">
              ...
              <p className="text-lg text-stone-600 font-light leading-relaxed max-w-md mx-auto md:mx-0">
                {col.metadata?.description || `Explore our ${col.title} collection`}
              </p>
              <div className="pt-4">
                <Link href={`/products?collection_id=${col.id}`} className="inline-flex items-center gap-2 text-stone-900 border-b border-stone-900 pb-1 hover:text-stone-600 hover:border-stone-600 transition-colors">
