    ...
    if (error || !order) {
        return (
            <div className="min-h-screen flex flex-col items-center justify-center p-4">
                ...
                <Link href="/account" className="underline">Back to Account</Link>
            </div>
        );
    }
            ...
            <div className="max-w-4xl mx-auto px-6">
                ...
                <div className="bg-white border border-stone-200 shadow-sm overflow-hidden">
                    <div className="p-6 md:p-8 border-b border-stone-100 flex flex-col md:flex-row md:items-center justify-between gap-4">
                        <div>
                            <h1 className="text-2xl font-serif text-stone-900 mb-1">Order #{order.display_id}</h1>
                            <p className="text-stone-500 text-sm flex items-center gap-2">
                                <Clock size={14} /> Placed on {date}
                            </p>
                        </div>
                        <div className={`px-4 py-1.5 rounded-full border text-xs font-bold uppercase tracking-widest inline-flex items-center gap-2 ${getStatusColor(order.status)}`}>
                    ...
                    <div className="bg-stone-50 p-6 border-b border-stone-100">
                        <div className="flex items-center justify-between text-xs font-bold uppercase tracking-widest text-stone-400">
                            ...
                            <span className={order.fulfillment_status !== 'not_fulfilled' ? 'text-stone-900' : ''}>Processing</span>
                            <span className={order.fulfillment_status === 'fulfilled' ? 'text-stone-900' : ''}>Shipped</span>
                            <span className={order.status === 'completed' ? 'text-stone-900' : ''}>Delivered</span>
                        </div>
                            ...
                            <div
                                ...
                                style={{
                                    width: order.status === 'completed' ? '100%' :
                                        order.fulfillment_status === 'fulfilled' ? '75%' :
                                            order.fulfillment_status !== 'not_fulfilled' ? '50%' : '25%'
                                }}
                            ></div>
                        </div>
                    </div>
                        ...
                        <div className="p-6 md:p-8 bg-stone-50/50 space-y-8">
                            <div>
                                ...
                                <address className="not-italic text-sm text-stone-600 space-y-1">
                                    <p className="font-medium text-stone-900">{order.shipping_address?.first_name} {order.shipping_address?.last_name}</p>
