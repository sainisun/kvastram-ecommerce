-- Kvastram DB Seed — run once on fresh Docker Postgres
...
(gen_random_uuid(),'store_name','"Kvastram"','general',now(),now()),
(gen_random_uuid(),'store_email','"support@kvastram.com"','general',now(),now()),
...
(gen_random_uuid(),'newsletter_title','"Join the Kvastram Family"','content',now(),now()),
(gen_random_uuid(),'seo_title','"Kvastram - Handcrafted Indian Fashion"','seo',now(),now()),
(gen_random_uuid(),'order_prefix','"KV"','orders',now(),now()),
(gen_random_uuid(),'social_instagram','"https://instagram.com/kvastram"','social',now(),now()),
